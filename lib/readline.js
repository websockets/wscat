...
function createInterface(options) {
  ...
  const search = createSearch(rl);
  const onKeypress = (data, key) => {
    if (!search(data, key)) rl.write(data, key);
  };
  const onEnd = () => {
    search(null, { ctrl: true, name: 'g' });
    input.end();
  };
  ...
  readline.emitKeypressEvents(source, rl);
...
function createSearch(rl) {
  ...
  return (data, key = {}) => {
    ...
    // handled normally by readline, including Ctrl+C to close the session.
    finish(false);
    return false;
  };
}
